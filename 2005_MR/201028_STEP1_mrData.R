...
#' hier z.b bmi sums suchen
#' ftp://ftp.ebi.ac.uk/pub/databases/gwas/summary_statistics/YengoL_30124842_GCST006900

# bmi_snp <- fread("/net/ifs2/san_projekte/projekte/genstat/02_projekte/1703_ge_metab_a1_b3_sorbs/1805_gxMetaboliteAssociation/dat/bmi_t2d_sum_stats/200508_smr_bmi.txt")
  ...
  res.snp <- tryCatch({
    ...
    my.snps <- sig.eqtl[
      ...
      .(gene.id,
        ...
        snp = paste(paste0("chr",chr_b37),
                    pos_b37,effect_allele_b37,
                    other_allele_b37,
                    sep=":"),
        snp.reverse = paste(paste0("chr",chr_b37),
                            pos_37,other_allele_b37,
                            effect_allele_b37,
                            sep=":"),
...
# pos chro without TAG -> use proxies of tag in case it is not in gtex data
...
top.mqtl.nontag <- top.mqtlfm[
  metabolite %in% v2$q3 & 
    logp>-log10(5e-8), 
  .(markername,r2_tagger,
    chr,cyto,chunk,pos,
    toppheno,metabolite,
    beta,se,i2,logp,tagger,
    cisgene)][
...
# get the top snp per phenotype
# pos chro
# DO NOT USE TAG=T! I want to be able to use proxies of tag snp in case the tag is not in gtex, but some related proxies are!
tag.mqtl <- top.mqtlfm[metabolite %in% sig.metabs & logp > -log10(5e-8),
                       .(tagsnp, markername, metabolite, toppheno,
                         markertoppheno, beta,se,i2,logp, 
                         effect_allele, other_allele,
...
tag.mqtl[,table(markername)][tag.mqtl[,table(markername)]>1]
tag.mqtl[,.N,by=toppheno]
tag.mqtl[tagsnp==TRUE, .N, metabolite] %>% as.data.frame()

# for each metab I have some TAGSNPs
venn2(
  tag.mqtl[tagsnp==TRUE, unique(metabolite)],
  tag.mqtl$metabolite %>% unique
)
...
mqtl <- unique(tag.mqtl[,.(chr,markername, pos, effect_allele, other_allele, tagsnp)])
length(mqtl)
mqtl$rs <- unlist(lapply(strsplit(mqtl$markername,split = ":",fixed = T),`[[`,1))

# LIFTOVER mqtl b37 -> gtex b38  ----

# create gtex style variant ID (including reverse mapping!)
tag.mqtl[, `:=`(
  variant_id_b37 = paste(chr, pos, effect_allele, other_allele, "b37", sep="_"),
  variant_id_rev_b37 = paste(chr, pos, other_allele, effect_allele, "b37", sep="_")
)]

# set keys for matching
setkey(tag.mqtl, variant_id_b37)
setkey(tag.mqtl, variant_id_rev_b37)
setkey(gtex8annot, variant_id_b37)

# check matching rs IDs in gtex and mqtl data
vrs <- venn2(tag.mqtl$rs.id,gtex8annot$rs_id_dbSNP151_GRCh38p7)

# check overlap 
vid <- venn3(
  tag.mqtl$variant_id_b37,
  tag.mqtl$variant_id_rev_b37,
  gtex8annot$variant_id_b37,
  mylabels=c("mqtl", "mqtl rev", "gtex")
)

# annotate matched SNP
tag.mqtl[, in_annot := (variant_id_b37 %in% vid$q2) | (variant_id_rev_b37 %in% vid$q3)]
tag.mqtl[in_annot==T, uniqueN(variant_id_b37)]
tag.mqtl[in_annot==T, id_reverse := ifelse(variant_id_b37 %in% (vid$q2), FALSE, TRUE)]
tag.mqtl[,.N]
tag.mqtl$in_annot %>% table
tag.mqtl$id_reverse %>% table

# match b38 ID to tag.mqtl (both!)

tag.mqtl$variant_id_b38 <- NULL

m1 <- match(tag.mqtl[in_annot==T & id_reverse == F, variant_id_b37], gtex8annot$variant_id_b37)
tmp <- gtex8annot[m1,paste(chr, variant_pos, ref, alt, "b38", sep="_")]
tag.mqtl[in_annot==T & id_reverse == F, variant_id_b38 := tmp] 
tag.mqtl[in_annot==T & id_reverse == F, ] %>% head

m2 <- match(tag.mqtl[in_annot==T & id_reverse==T, variant_id_rev_b37], gtex8annot$variant_id_b37)
tmp <- gtex8annot[m2,paste(chr, variant_pos, ref,alt, "b38", sep="_")]
tmp %>% head
tag.mqtl[in_annot==T & id_reverse == T, variant_id_b38 := tmp] 
tag.mqtl[in_annot==T & id_reverse == T, variant_id_b38] %>% head()

# check 
check1 <- venn2(tag.mqtl[in_annot==T, variant_id_b38],
                gtex8annot$variant_id)
stopifnot(length(check1$q1) == length(vid$q2) + length(vid$q3))



gtex.all.eqtl[gene.id %in% unique(lookup.mqtl$gene), unique(chr) ,gene.id]


tag.mqtl[in_annot==T, paste(unique(chr), collapse=", "),by=metabolite]

# top raussuchen


# diese snp sind in gtex vorhanden
top.mqtlfm[grep(paste(unique(lookup.mqtl$gene), collapse = "|"),cisgene), ][
  metabolite %in% unique(lookup.mqtl$metabolite), table(cyto)]


top.mqtlfm[grep(paste(unique(lookup.mqtl$gene), collapse = "|"),transgene), ][
  metabolite %in% unique(lookup.mqtl$metabolite), table(cyto)]
...
tosearch <- gtex.all.eqtl[gene.id %in% unique(lookup.mqtl$gene), ][,.SD[pval_nominal==min(pval_nominal)],gene.id][,variant_id]
gtex8annot[variant_id %in% (tosearch),]
...
# check for genes in full gtex data
v <- venn2(unique(lookup.mqtl$gene), unique(gtex.all.eqtl$gene.id))
gtex.all.eqtl[gene.id %in% unique(lookup.mqtl$gene), .N]

tag.mqtl[in_annot==T,variant_id_b38] %>% head
gtex.all.eqtl[gene.id %in% unique(lookup.mqtl$gene), variant_id] %>% head

v1<-venn2(
  # tag.mqtl[in_annot==T,paste(paste0("chr", chr), pos, other_allele,effect_allele,"b38",sep="_")],
  # tag.mqtl[in_annot==T,paste(paste0("chr", chr), pos, effect_allele,other_allele,"b38",sep="_")],
  tag.mqtl[in_annot==T,variant_id_b38],
  gtex.all.eqtl[, variant_id]
  # gtex.all.eqtl[gene.id %in% unique(lookup.mqtl$gene), variant_id]
)
