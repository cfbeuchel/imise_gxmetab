...
# Enrichment Of Strong Ratio Associations ----
#' ## Enrichment Of Strong Ratio Associations
...
# get fg
res.ratio <- res.meta[grepl("^Q", metabolite) & significant == TRUE, ]
res.ratio <- res.ratio[order(-abs(betaREM)),.SD,metabolite]
res.ratio[,.N,metabolite]
res.ratio[,head(.SD,5),metabolite]

# select only the 100 strongest gene per ratio
res.ratio <- res.ratio[,head(.SD,100),metabolite]

# create foreground for enrichment per ratio
assoc.fg <- res.ratio[!is.na(entrez.id), 
                     unique(entrez.id), 
                     by=metabolite]
...
res.go[,data.base := "GO"]
res.go[!is.na(qvalue) & qvalue<=0.05][order(qvalue), head(.SD,1),Cluster]
res.go[!is.na(qvalue) & qvalue<=0.05,table(Description)] %>% sort()

# viz
dotplot(assoc.enrich.go)

# merge proper names and save
m1 <- match(res.go$Cluster, agn.all$rid)
res.go$`Metabolite Abbreviation` <- agn.all[m1, abbr]
res.go$`Metabolite` <- agn.all[m1, full.name]
res.go <- res.go[,.(
  `Pathway data base` = data.base,
  `Pathway ID` = ID, 
...
WriteXLS_hk(
  res.go,
  ExcelFileName = "ppr/supp_tables/201119_ratioGeneEnrich.xls",
  SheetNames = "Metabolite Ratio GO Enrichment"
  )

# Enrichment of BMI-mediations ----
#' ## Enrichment of BMI-mediations

# load the network data
bmi_genes <- read_xls(sheet = 1,path = "ppr/main_tables/201116_bmiMediationTable.xls")
setDT(bmi_genes)

# match entrez ids to data
bmi_genes$entrez.id <- NULL
m1 <- match(bmi_genes$Gene, entrez.map[ilmn_mapping_hg19v2 %in% entrez.qc[1:8],symbol_INGENUITY])
bmi_genes <- cbind(bmi_genes, entrez.map[ilmn_mapping_hg19v2 %in% entrez.qc[1:8],][m1, .(entrez.id = ilmn_entrezID_INGENUITY)])


# get fg
assoc.fg <- bmi_genes[!is.na(entrez.id), 
                     unique(entrez.id), 
                     by=Metabolite]
...
# enrichment analysis
assoc.enrich.go <- compareCluster(
  geneCluster = list("all" = unlist(assoc.fg)), 
  fun = "enrichGO", 
  OrgDb = org.Hs.eg.db,
  universe = assoc.bg, 
  minGSSize = 2,
